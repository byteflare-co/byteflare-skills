...
3. 配置先が git 管理対象か確認し、必要であれば案内する
...
- `.claude/skills/playbook/references/ui-map.md` — UI Map と Playbook の索引
- `.claude/skills/playbook/references/[name].md` — 個別の Playbook ファイル

スキル本体とデータが同じディレクトリにあるため、配置先が git 管理されていれば自動的にチーム共有できます（追加の gitignore 調整は不要）。
